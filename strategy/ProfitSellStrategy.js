...
class ProfitSellStrategy extends BaseStrategy {
    ...
    _profitShort(tick, num) {
        let position = this.GetPosition(tick.symbol);
        let unFinishOrderList = this.GetUnFinishOrderList();
        if (position !== undefined) {
            let todayShortPositions = position.GetShortTodayPosition();
            let shortPositionAveragePrice = position.GetShortPositionAveragePrice();
            let availableNum = todayShortPositions - unFinishOrderList.length;
            if (!num) {
                num = availableNum;
            }
            console.log("num: " + num + " todayShortPositions: " + todayShortPositions + " unFinishOrderList.length: " + unFinishOrderList.length);
            if (tick.lastPrice < shortPositionAveragePrice && this.lastOpenPrice && this.lastOpenPrice > tick.lastPrice) {
        ...
        if (isTimeToClose === 2) {
            ...
            if (this.flag === true) {
                if (this.direction === "short") {
                    ...
                    let unLockShortPosition = this.GetUnLockShortPosition();
                    let sum = this._getAvilableSum(tick);
                    if (sum >= 1 && this.action === true && unLockShortPosition < 1) {
...
module.exports = ProfitSellStrategy;
